// Builds the chatbot's knowledge base from the site's curated content.
//
// The corpus is small enough to live entirely in the model's system prompt, so
// there is no retrieval step: this script flattens each page to readable text
// and emits one module the Worker imports. Keeping the source of truth as the
// published pages and explicitly approved assistant-only notes is deliberate:
// every claim remains traceable to content Kwamina controls.

import { readFile, writeFile, mkdir } from 'node:fs/promises'
import { createHash } from 'node:crypto'
import { dirname, join } from 'node:path'
import { fileURLToPath } from 'node:url'
import { ROUTE_PATHS } from '../src/routes.js'

const root = join(dirname(fileURLToPath(import.meta.url)), '..')
const outputPath = join(root, 'src/generated/corpus.js')

// Ordered most- to least-general, which is also the order the model reads them.
const SOURCES = [
  { file: 'docs/content/homepage.html', path: ROUTE_PATHS.home, title: 'Homepage' },
  { file: 'docs/content/about.html', path: ROUTE_PATHS.about, title: 'About' },
  { file: 'docs/content/personal-notes.md', path: null, title: 'Personal notes' },
  { file: 'docs/content/work/athena/index.html', path: ROUTE_PATHS.athena, title: 'Athena — product story' },
  { file: 'docs/content/work/athena/local-first-pos/index.html', path: ROUTE_PATHS.localFirstPos, title: 'Athena — local-first point of sale' },
  { file: 'docs/content/work/athena/agent-ready-repository/index.html', path: ROUTE_PATHS.agentReadyRepository, title: 'Athena — agent-ready repository' },
  { file: 'docs/content/work/athena/read-optimized-reporting/index.html', path: ROUTE_PATHS.readOptimizedReporting, title: 'Athena — read-optimized reporting' },
  { file: 'docs/content/work/athena/if-it-matters-make-it-a-gate/index.html', path: ROUTE_PATHS.ifItMattersMakeItAGate, title: 'Athena — If It Matters, Make It a Gate' },
  { file: 'docs/content/work/athena/valid-but-not-this-ticket/index.html', path: ROUTE_PATHS.validButNotThisTicket, title: 'Athena — valid, but not this ticket' },
  // Assistant-facing only: describes the site's own engineering, including the
  // assistant itself. Deliberately unpublished — path: null keeps it out of the
  // page allowlist, and the emitted document says it has no public page.
  { file: 'docs/content/work/this-site.md', path: null, title: 'How this site is built' },
]

// Past this the system prompt stops being obviously cheap to send on every
// request and retrieval starts to earn its complexity. A warning rather than a
// failure: the build should not break because a page grew.
const TOKEN_BUDGET = 40_000

const ENTITIES = {
  amp: '&', lt: '<', gt: '>', quot: '"', apos: "'", nbsp: ' ',
  mdash: '—', ndash: '–', hellip: '…', rsquo: '’', lsquo: '‘',
  rdquo: '”', ldquo: '“', times: '×', middot: '·',
}

function decodeEntities(text) {
  return text
    .replace(/&#(\d+);/g, (_, code) => String.fromCodePoint(Number(code)))
    .replace(/&#x([0-9a-f]+);/gi, (_, code) => String.fromCodePoint(parseInt(code, 16)))
    .replace(/&([a-z]+);/gi, (match, name) => ENTITIES[name] ?? match)
}

// Alt text carries real content on the Athena pages — the workspace captures
// are described there and nowhere else — so images become a caption line
// rather than being dropped with the rest of the markup.
function htmlToText(html) {
  const body = html.match(/<body[^>]*>([\s\S]*?)<\/body>/i)?.[1] ?? html

  const text = body
    .replace(/<(script|style|svg|noscript)\b[\s\S]*?<\/\1>/gi, ' ')
    .replace(/<img\b[^>]*\balt=["']([^"']+)["'][^>]*>/gi, '\n[image: $1]\n')
    .replace(
      /<a\b[^>]*\bhref=["'](https?:\/\/[^"']+|\/docs\/[^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi,
      (_, href, label) => `${label.replace(/<[^>]+>/g, '').trim()} (${href})`,
    )
    .replace(/<h([1-6])\b[^>]*>/gi, (_, level) => `\n\n${'#'.repeat(Number(level))} `)
    .replace(/<\/h[1-6]>/gi, '\n')
    .replace(/<li\b[^>]*>/gi, '\n- ')
    .replace(/<(p|div|section|article|header|footer|tr|figcaption|blockquote)\b[^>]*>/gi, '\n')
    .replace(/<\/(p|div|section|article|header|footer|tr|figcaption|blockquote|ul|ol|li)>/gi, '\n')
    .replace(/<(td|th)\b[^>]*>/gi, ' | ')
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/<[^>]+>/g, '')

  return collapse(decodeEntities(text))
}

function collapse(text) {
  return text
    .split('\n')
    .map((line) => line.replace(/[ \t ]+/g, ' ').trim())
    .join('\n')
    .replace(/\n{3,}/g, '\n\n')
    .trim()
}

async function buildSection({ file, path, title }) {
  const raw = await readFile(join(root, file), 'utf8')
  const content = file.endsWith('.md') ? collapse(raw) : htmlToText(raw)
  // The page path lets the assistant point a reader at the page an answer came
  // from, which is the main thing that makes its answers checkable.
  const location = path ? `Page: ${path}` : 'Source: internal notes, no public page'

  return `<document title="${title}">\n${location}\n\n${content}\n</document>`
}

const sections = await Promise.all(SOURCES.map(buildSection))
const corpus = sections.join('\n\n')
const corpusVersion = createHash('sha256').update(corpus).digest('hex').slice(0, 12)

// The pages the assistant can be told a reader is looking at. Emitted from the
// same list that built the corpus so the two cannot disagree, and used by the
// Worker as an allowlist: the browser says which path it is on, and only a
// match against this table — never the browser's own string — reaches the
// model. A page path is untrusted input like any other.
const pages = SOURCES.filter((source) => source.path).map(({ path, title }) => ({ path, title }))

await mkdir(dirname(outputPath), { recursive: true })
await writeFile(
  outputPath,
  [
    '// Generated by scripts/build-corpus.mjs — do not edit.',
    `export default ${JSON.stringify(corpus)}`,
    `export const CORPUS_VERSION = ${JSON.stringify(corpusVersion)}`,
    `export const PAGES = ${JSON.stringify(pages, null, 2)}`,
    '',
  ].join('\n'),
)

// Roughly four characters per token; precise enough to catch a corpus that has
// outgrown the no-retrieval assumption.
const estimatedTokens = Math.round(corpus.length / 4)
console.log(`corpus: ${sections.length} documents, ${corpus.length} chars, ~${estimatedTokens} tokens`)
if (estimatedTokens > TOKEN_BUDGET) {
  console.warn(`corpus: ~${estimatedTokens} tokens exceeds the ${TOKEN_BUDGET} budget — consider retrieval instead of a whole-corpus system prompt`)
}
